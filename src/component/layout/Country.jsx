...
import { faCirclePlus, faEye,faPenToSquare, faTrash ,faArrowRight} from '@fortawesome/free-solid-svg-icons'
...
const Country = () => {
  ...
  useEffect(() => {
    ...
    const fetchCountry = async () => {
      const {data} = await axios.post('/countries/view', country) 
      console.log(data)
...
const handleSubmit = async (e) => {
  e.preventDefault();
  if(id === 'new'){
    let response = await axios.post('/countries/create',JSON.stringify(country))
    console.log(response)
    setCountry({})
    return navigate('/admin/countriesList')
  }else{
    let response = await axios.post('/countries/update',JSON.stringify(country))
    console.log(response)
    setCountry({})
    return navigate('/admin/countriesList')
  }
